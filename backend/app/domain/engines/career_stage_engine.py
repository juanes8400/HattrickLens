...
def classify_career_stage(
    ...
    signals: dict[str, Any] = {
        "ageYears": age_years, "ageDays": age_days, "ageFactor": af,
        "skillsRising": skills_rising, "skillsFalling": skills_falling,
        "skillsStable": skills_stable, "hasSufficientHistory": has_sufficient_history,
        "squadPercentile": squad_percentile, "leadership": leadership, "loyalty": loyalty,
    }
    ...
    if age_years <= PEAK_MAX_AGE:
        if squad_percentile is not None and squad_percentile >= 60:
            return CareerStage(
                ...
                confidence="alta", signals=signals,
            )
    ...
    if skills_falling > skills_rising and af < 0.85:
        return CareerStage(
            stage="declive", label="En declive",
            rationale=(
    ...
    return CareerStage(
        stage="veterano", label="Veterano estable",
        rationale=(
