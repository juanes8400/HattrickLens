...
def test_plumber_is_detected() -> None:
    """Con TODO revelado se puede decir que no va a dar mas.

    2026-08-30: la frase ya no manda despedir. Despedir libera una plaza, asi
    que es una decision de plantilla --a quien suelto de los dieciocho-- y
    vive en `quien_sobra`. Aqui solo se constata el hecho.
    """
    e = evaluate(
        "Fontanero",
        17,
        0,
        # Las SIETE: un veredicto firme exige que no quede nada por mirar.
        _skills(
            keeper=(1, 2),
            defending=(2, 3),
            playmaking=(1, 2),
    ...
    assert "no va a dar más" in e.promote_advice
...
def test_ready_to_promote_at_seventeen() -> None:
    e = evaluate(
        ...
        17,
        50,
        # Las siete, para que el consejo llegue al tramo de la edad en vez de
        # quedarse en «aun le faltan habilidades por revelar».
        _skills(
            keeper=(2, 3),
            defending=(6, 8),
            playmaking=(9, 11),
...
def test_training_exposure_uses_hattrick_control_weights() -> None:
    ...
    assert training_exposure(90, 0, True, False, ("passing", "passing_defenders")) == pytest.approx(
        2 / 3, abs=0.0001
    )
    # Si es exactamente el mismo entrenamiento, el secundario normal se
    # castiga a la mitad: 2/3 x 1/2 = 1/3.
    assert training_exposure(90, 0, True, False, ("passing", "passing")) == pytest.approx(
        1 / 3, abs=0.0001
    )
...
def test_no_se_condena_a_nadie_con_techos_sin_revelar() -> None:
    """La regla que rompio el consejo viejo.

    Bastaban TRES habilidades reveladas de siete para recomendar el despido, y
    eso hacia dos cosas al reves: condenaba a quien tenia mas desconocidas que
    conocidas, y como revelar es lo que pasa al entrenar, cuanto mas sabias de
    un chico mas cerca estaba de que te dijeran que lo echaras.
    """
    a_medias = evaluate(
        "Visto a medias",
        16,
    ...
    assert a_medias.revealed_skills == 3
    assert "por revelar" in a_medias.promote_advice
    assert "no va a dar más" not in a_medias.promote_advice

    # Con las SIETE reveladas ya no hay sorpresa posible: ahi si se dice.
    todo = evaluate(
        "Visto de sobra",
        16,
        0,
    ...
    assert todo.revealed_skills == 7
    assert "no va a dar más" in todo.promote_advice
