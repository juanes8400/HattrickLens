...
export function staffEffectLines(effect: ClubStaffRoleEffect): string[] {
  ...
  if (effect.trainingSpeedPct != null)
    lines.push(`Velocidad de entrenamiento +${effect.trainingSpeedPct}%`);
  if (effect.recoverySpeedPct != null)
    lines.push(`Velocidad de recuperación +${effect.recoverySpeedPct}%`);
  if (effect.backgroundForm != null && effect.backgroundForm !== 0)
    lines.push(`Forma de fondo +${effect.backgroundForm}`);
  if (effect.injuryRiskPp != null && effect.injuryRiskPp !== 0)
    lines.push(`Riesgo de lesión +${effect.injuryRiskPp} pp`);
  if (
    effect.injuryRiskReductionPp != null &&
    effect.injuryRiskReductionPp !== 0
  )
    lines.push(`Riesgo de lesión −${effect.injuryRiskReductionPp} pp`);
  if (effect.teamSpirit != null && effect.teamSpirit !== 0)
    lines.push(`Espíritu del equipo +${effect.teamSpirit}`);
  if (effect.confidence != null && effect.confidence !== 0)
    lines.push(`Confianza +${effect.confidence}`);
  if (effect.maxFunds != null)
    lines.push(`Fondos máximos ${money(effect.maxFunds, "US$")}`);
  if (effect.weeklyReturn != null)
    lines.push(`Retorno semanal ${money(effect.weeklyReturn, "US$")}`);
  if (effect.extraOrders != null && effect.extraOrders !== 0)
    lines.push(`+${effect.extraOrders} órdenes extra`);
  if (effect.styleFlexibilityPp != null && effect.styleFlexibilityPp !== 0)
    lines.push(`Flexibilidad de estilo +${effect.styleFlexibilityPp} pp`);
  return lines;
}
