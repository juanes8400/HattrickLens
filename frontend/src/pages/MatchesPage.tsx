  ...
  return (
    <div className="space-y-4">
      ...
      <header className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h1 className="text-xl font-semibold">{tx("Partidos")}</h1>
          <p className="text-sm text-[var(--muted)]">
            {tx("Por qué se ganó o se perdió")}
          </p>
          <EnlaceATransparencia seccion="partidos" calculo="hatstats" />
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <select
            aria-label={tx("Filtrar los partidos por temporada")}
            value={season ?? "all"}
            onChange={(e) =>
              setSeason(
          ...
          >
            {includeFriendlies
              ? tx("Ocultar amistosos")
              : tx("Mostrar amistosos")}
          </button>
          {missingDetails > 0 && (
            ...
            >
              {missingDetails} {tx("sin ratings · se completan al sincronizar")}
            </Link>
          )}
        </div>
      </header>
      ...
      <Panel
        title={tx("Resumen")}
        meta={tx("local/visitante y mejores marcas")}
      >
      ...
      <Panel
        title={tx("Conversión")}
        meta={tx("generación frente a definición")}
      >
      ...
      {data.ratingSeries.length > 0 && (
        <Panel
          title={tx("Evolución de los ratings")}
          meta={tx("{{v0}} partidos · tt-ss", { v0: data.ratingSeries.length })}
        >
...
function ResumenPanel({ data }: { data: Matches }) {
  const homeAwayColumns: Column<HomeAwayRow>[] = [
    { key: "label", header: tx("Ámbito"), value: (r) => r.label },
    {
      key: "played",
      header: tx("Partidos"),
      align: "right",
    ...
    { key: "won", header: tx("Ganados"), align: "right", value: (r) => r.won },
    {
      key: "drawn",
      header: tx("Empatados"),
      align: "right",
    ...
    { key: "gf", header: tx("Gf"), align: "right", value: (r) => r.goalsFor },
    {
      key: "ga",
      header: tx("Gc"),
      align: "right",
  ...
  const bestColumns: Column<BestRating>[] = [
    { key: "label", header: tx("Métrica"), value: (r) => r.label },
    {
      key: "value",
      header: tx("Mejor valor"),
      align: "right",
      value: (r) => r.value,
    },
    { key: "date", header: tx("Fecha"), value: (r) => r.date },
    {
      key: "opponent",
      header: tx("Partido"),
      value: (r) => r.opponent,
  ...
  return (
    <div className="grid gap-4 p-4 lg:grid-cols-2 [&>*]:min-w-0">
      <div className="space-y-4">
        <DataTable
          emptyMessage={tx(
            "Sin partidos jugados como local ni como visitante.",
          )}
          rows={data.homeAway}
          ...
          filterPlaceholder={tx("Filtrar…")}
        />
        <Chart
          ariaLabel={tx(
            "Distribución de resultados: ganados, empatados y perdidos",
          )}
      ...
      <div>
        <h3 className="mb-2 text-xs font-medium uppercase tracking-wide text-[var(--muted)]">
          {tx("Mejores calificaciones")}
        </h3>
        {data.bestRatings.length ? (
          <DataTable
            emptyMessage={tx("Sin valoraciones guardadas todavía.")}
          ...
          <p className="text-xs text-[var(--muted)]">
            {tx("Todavía no hay ratings por sector sincronizados.")}
          </p>
        )}
      </div>
    </div>
  ...
  if (!c.ownChances && !c.opponentChances) {
    return (
      <p className="p-4 text-xs text-[var(--muted)]">
        {tx(
          "Todavía no hay ocasiones por zona sincronizadas, así que no se puede calcular la conversión. Llegan con el detalle del partido.",
        )}
      </p>
    );
  }
    ...
    <div className="space-y-4 p-4">
      <div className="grid gap-4 sm:grid-cols-2 [&>*]:min-w-0">
        <div className="rounded-lg border border-[var(--border)] p-3">
          <div className="text-xs text-[var(--muted)]">{tx("Nosotros")}</div>
          <div
            className={
              c.isReliable
          ...
          <div className="mt-1 text-xs text-[var(--muted)]">
            {c.ownGoals} {tx("de")} {c.ownChances} {tx("ocasiones")}
            {!c.isReliable && (
              <span className="block text-[var(--danger)]">
                {tx("muestra corta: es ruido")}
              </span>
            )}
          </div>
        </div>
        <div className="rounded-lg border border-[var(--border)] p-3">
          <div className="text-xs text-[var(--muted)]">{tx("Rival")}</div>
          <div
            className={
              c.isReliable
                ? "mt-2 text-2xl font-semibold tabular-nums"
          ...
          <div className="mt-1 text-xs text-[var(--muted)]">
            {c.opponentGoals} {tx("de")} {c.opponentChances} {tx("ocasiones")}
          </div>
        </div>
      </div>
      <div>
        <h3 className="mb-2 text-xs font-medium uppercase tracking-wide text-[var(--muted)]">
          {tx("Ocasiones por zona")}
        </h3>
        ...
        <Chart
          ariaLabel={tx(
            "Ocasiones generadas y concedidas por zona de la cancha",
          )}
...
${p.opponent.length > 14 ? `${p.opponent.slice(0, 13)}…` : p.opponent}`;
  ...
  return (
    <Chart
      ariaLabel={tx(
        "Evolución de los ratings de mediocampo, defensa y ataque por partido",
      )}
      option={{
        xAxis: {
          type: "category",
...
function MatchTable({
  ...
  const columns: Column<MatchRow>[] = [
    ...
    { key: "tournament", header: tx("Torneo"), value: (r) => r.tournament },
    {
      key: "result",
      header: tx("Resultado"),
  ...
  return (
    <DataTable
      emptyMessage={tx(
        "Ningún partido jugado. Sincroniza para traer el historial.",
      )}
      rows={data.matches}
      columns={columns}
...
function MatchDetailPanel({ htMatchId }: { htMatchId: number }) {
  ...
  if (isLoading)
    return (
      <Panel title={tx("Análisis del partido")}>
        <Loading />
      </Panel>
    );
  if (isError || !data) {
    return (
      <Panel title={tx("Análisis del partido")}>
        <Empty>{tx("Sin ratings por sector.")}</Empty>
      </Panel>
    );
  }
      ...
      <div className="grid gap-4 p-4 lg:grid-cols-2 [&>*]:min-w-0">
        ...
        <div className="space-y-3 text-xs leading-relaxed text-[var(--muted)]">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <div className="text-[var(--muted)]">{tx("HatStats")}</div>
            ...
            <div>
              <div className="text-[var(--muted)]">{tx("Posesión")}</div>
              <div className="text-lg tabular-nums text-[var(--text)]">
            ...
            {hasChances && (
              <div>
                <div className="text-[var(--muted)]">{tx("Conversión")}</div>
                <div className="text-lg tabular-nums text-[var(--text)]">
                ...
                <div className="text-[10px] text-[var(--muted)]">
                  {data.ownChances.goals}/{data.ownChances.total}{" "}
                  {tx("ocasiones propias ·")} {data.opponentChances.goals}/
                  {data.opponentChances.total} {tx("rivales")}
                </div>
              </div>
            )}
          </div>
          {data.strengths.length > 0 && (
            <p>
              <b className="text-[var(--positive)]">{tx("Dominamos:")}</b>{" "}
              {data.strengths.join(", ")}.
            </p>
          )}
          {data.weaknesses.length > 0 && (
            <p>
              <b className="text-[var(--danger)]">{tx("Nos superaron en:")}</b>{" "}
          ...
          {!data.strengths.length && !data.weaknesses.length && (
            <p>
              {tx(
                "Ningún sector se decidió por más de 10 puntos: partido equilibrado.",
              )}
            </p>
          )}
        </div>
