...
export function LibroDeVisitasPage() {
  ...
  return (
    <div className="space-y-4">
      <header>
        <h1 className="text-xl font-semibold">{tx("Libro de visitas")}</h1>
        <p className="prosa text-sm text-[var(--muted)]">
          {tx(
            "Cuéntame qué te está sirviendo y qué te falta. De aquí salen las funcionalidades siguientes.",
          )}
        </p>
      </header>

      <Panel title={tx("Deja tu mensaje")}>
        <div className="space-y-3 p-4">
          <label className="block">
            <span className="sr-only">{tx("Tu mensaje")}</span>
            <textarea
              value={texto}
              onChange={(e) => setTexto(e.target.value)}
              rows={4}
              maxLength={MAXIMO}
              placeholder={tx("Qué usas, qué te falta, qué te sobra…")}
              className="w-full rounded-md border border-[var(--border)] bg-[var(--bg)] px-3 py-2 text-sm text-[var(--text)]"
            />
          </label>
          <div className="flex flex-wrap items-center justify-between gap-3">
            <span className="text-xs text-[var(--muted)]">
              {tx("Firmas con el nombre de tu club, no con el de tu cuenta.")}
            </span>
            <div className="flex items-center gap-3">
              ...
              >
                {firmar.isPending ? tx("Enviando…") : tx("Firmar")}
              </button>
            </div>
          </div>
          {firmar.isError && (
      ...
      <Panel title={tx("Firmas")} meta={data ? `${data.entries.length}` : ""}>
        {isLoading ? (
          <Loading />
        ) : isError ? (
              ...
              <li key={f.id} className="px-4 py-3">
                <div className="flex flex-wrap items-baseline gap-x-2">
                  <span className="text-sm font-medium">
                    {f.teamName || tx("Un club sin nombre todavía")}
                  </span>
                  {f.country && (
                    <span className="text-xs text-[var(--muted)]">
                      {f.country}
          ...
          <p className="prosa p-4 text-sm text-[var(--muted)]">
            {tx("Todavía no ha firmado nadie. Estrénalo tú.")}
          </p>
        )}
      </Panel>
    </div>
