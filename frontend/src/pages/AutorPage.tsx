...
const DIVULGACION: { titulo: string; fecha: string; url: string }[] = [
  {
    titulo: tx("Why Bayesian Statistics Are Useless"),
    fecha: "julio 2024",
    url: "https://juandelacalle.medium.com/why-bayesian-statistics-are-useless-86c0260520c3",
  },
  {
    titulo: tx("Why Agile Doesn't Work for Data Science, And That's Okay"),
    fecha: "octubre 2023",
  ...
  {
    titulo: tx("HalvingGridSearch: An Experimental Framework for Fine Tuning"),
    fecha: "julio 2023",
  ...
  {
    titulo: tx("10 Hidden Stories Found in a ROC Curve"),
    fecha: "julio 2023",
...
function ApoyoPanel() {
  ...
  return (
    <Panel title={tx("Apoyar el proyecto")}>
...
export function AutorPage() {
  return (
    <div className="space-y-4">
      <header>
        <h1 className="text-xl font-semibold">{tx("Autor")}</h1>
        <p className="text-sm text-[var(--muted)]">
          {tx("Quién hace HT Lens, y qué ha publicado.")}
        </p>
      </header>
      ...
      <Panel title={tx("Juan Esteban de la Calle")}>
        <a
        ...
        >
          <ImagenOpcional
            src={ESCUDO}
            alt={tx("Escudo de Pulgas Arrechas")}
            width={72}
            height={72}
        ...
        <div className="flex flex-col gap-5 p-4 sm:flex-row sm:items-start">
          ...
          <img
            src={autor}
            alt={tx("Retrato de Juan Esteban de la Calle")}
            width={112}
            height={112}
            loading="lazy"
          ...
          <div className="prosa space-y-3 text-sm leading-relaxed">
            ...
            <p>
              {tx(
                "Ingeniero matemático. Ha modelado el oxígeno disuelto de un embalse, lo que cuesta y lo que salva tratar a un paciente después de un infarto, el inventario de una fábrica de plásticos y el orden en que una planta debe sacar sus lotes.",
              )}
            </p>
            <p>
              {tx(
                "Son problemas que no se parecen en nada y se atacan casi igual: mirar qué varía de verdad, escribir el modelo y dejarlo equivocarse contra los datos hasta que deja de hacerlo. Sobre estadística escribe sin mucha diplomacia: tiene un artículo titulado «Por qué la estadística bayesiana es inútil» y se declara enemigo número uno del SMOTE.",
              )}
            </p>
            <p>
              {tx(
                "HT Lens es esa misma manía aplicada a un club de Hattrick. Empezó siendo una hoja de cálculo, se hizo demasiado grande, y acabó aquí.",
              )}
            </p>
      ...
      <Panel
        title={tx("Publicaciones científicas")}
        meta={tx("{{v0}} referencias", { v0: CIENTIFICAS.length })}
      >
      ...
      <Panel
        title={tx("Divulgación")}
        meta={tx("{{v0}} artículos", { v0: DIVULGACION.length })}
      >
        <ul className="divide-y divide-[var(--border)]">
          {DIVULGACION.map((a) => (
