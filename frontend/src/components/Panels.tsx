...
export function Loading() {
  ...
  return (
    ...
    >
      <span className="sr-only">{t("comun.cargando", "Cargando…")}</span>
    ...
    return (
      <div className="max-w-lg rounded-lg border border-[var(--warning)] bg-[var(--surface)] p-6">
        <h2 className="font-semibold">
          {t("comun.sesionVencidaTitulo", "Tu sesión de Hattrick venció")}
        </h2>
...
function ErrorConReintento({ error }: { error: unknown }) {
  ...
  return (
    <div className="rounded-lg border border-[var(--danger)] bg-[var(--surface)] p-6">
      <h2 className="font-semibold">
        {t("comun.errorTitulo", "No pudimos cargar estos datos")}
      </h2>
      <p className="prosa mt-3 text-sm text-[var(--muted)]">
        {t(
          "comun.errorTexto",
          "Puede ser un tropiezo momentáneo. Si vuelve a fallar, revisa que la conexión local de HT Lens siga en pie.",
        )}
      </p>
      <button
        type="button"
      ...
      >
        {reintentando
          ? t("comun.reintentando", "Reintentando…")
          : t("comun.reintentar", "Reintentar")}
      </button>
...
export function SinDatos() {
  ...
  return (
    <div className="rounded-lg border border-[var(--border)] bg-[var(--surface)] p-6">
      <h2 className="font-semibold">
        {t("comun.sinDatosTitulo", "Aquí no llegó nada")}
      </h2>
      <p className="prosa mt-1 text-sm text-[var(--muted)]">
        {t(
          "comun.sinDatos",
          "La pantalla pidió sus datos y se quedó sin respuesta. Suele ser que la conexión local de HT Lens no está en pie.",
        )}
      </p>
      <button
        type="button"
